...
-- ─── CORRECTIFS INSCRIPTION ─────────────────────────────────
-- Le trigger handle_new_user tourne en security definer → peut écrire profiles sans RLS
-- Mais si le trigger est lent, l'API register a besoin d'upsert le profil
-- La service_role_key bypasse RLS → pas de policy supplémentaire nécessaire

-- Autoriser l'insertion de profil par le propriétaire lui-même
-- (utile si le trigger n'a pas encore tourné et que le client fait un insert direct)
